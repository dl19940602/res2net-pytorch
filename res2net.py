...
#        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
    ...
    def _make_layer(self, block, planes, width, blocks, stride=1, scales=4, groups=1, se=False, norm_layer=None):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, width, downsample, stride=stride, scales=scales, groups=groups, se=se, norm_layer=norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, width, scales=scales, groups=groups, se=se, norm_layer=norm_layer))
...
#        planes = [int(width * scales * 2 ** i) for i in range(3)]
        ...
        self.bn1 = norm_layer(64)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(Res2NetBottleneck, 64, 24, layers[0], scales=scales, groups=groups, se=se, norm_layer=norm_layer)
        self.layer2 = self._make_layer(Res2NetBottleneck, 128, 48, layers[1], stride=2, scales=scales, groups=groups, se=se, norm_layer=norm_layer)
        self.layer3 = self._make_layer(Res2NetBottleneck, 256, 96, layers[2], stride=2, scales=scales, groups=groups, se=se, norm_layer=norm_layer)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(256 * Res2NetBottleneck.expansion, num_classes)
